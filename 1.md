...
Flask-CI-CD/ │ ├── ansible/ # Ansible automation │ ├── inventory.ini # Inventory file for Ansible (target host) │ ├── playbook.yml # Main playbook to set up app and monitoring │ └── monitoring/ # Monitoring configuration │ ├── docker-compose.yml # Docker Compose config for Prometheus, Grafana, Node Exporter │ └── prometheus.yml # Prometheus scrape configuration │ ├── terraform/ # Terraform code for infrastructure │ ├── main.tf # AWS resources definition (VPC, EC2, ALB, etc.) │ ├── variables.tf # Input variables │ ├── outputs.tf # Outputs like public IP, ALB DNS │ └── terraform.tfvars # Actual values of the input variables │ ├── app/ # Flask application (source code) │ ├── Dockerfile # Dockerfile for Flask app │ ├── app.py # Flask application code │ └── requirements.txt # Python dependencies │ ├── .gitignore # Files and folders ignored by Git ├── README.md # Project documentation └── LICENSE # Project license
